...
const PREFER_LOCAL_GRAPHICS = false;
const PREFER_LOCAL_NPCS = false;
const PREFER_LOCAL_BODIES = false;
const PREFER_LOCAL_SPELLS_ASSETS = false;
...
function buildGraphicFrames(
    ...
    if (frameIds !== undefined) {
        for (let index = 0; index < frameIds.length; index++) {
            ...
            if (fid !== undefined) frames[(index + 1).toString()] = fid.toString();
        }
...
export function getMapDimensions(
    ...
    const mapEntry = mapData[mapNumber];
    if (!mapEntry) return { width: 100, height: 100 };
    const yKeys = Object.keys(mapEntry)
        .map(Number)
        .sort((a, b) => a - b);
    const firstY = yKeys[0];
    const xKeys = Object.keys((firstY !== undefined ? mapEntry[firstY] : undefined) ?? {})
        .map(Number)
        .sort((a, b) => a - b);
